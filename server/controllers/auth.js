...
export const register = async (req, res) => {
  try {
    ...
    const user = await new User({
      username: email.split("@")[0] + Math.random().toString(36).slice(-2),
      email,
...
export const login = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.json({
        error: "Could not find user with this email, please register.",
      });
    }
    // 2 compare password
...
export const profile = async (req, res) => {
  try {
    ...
    console.log(err);
    return res.json({ error: "user with this username could not be found." });
  }
};
...
export const updateProfile = async (req, res) => {
  try {
    ...
    if (err.codeName === "DuplicateKey") {
      ...
      return res.json({ error: "Unauhorized" });
    }
  }
};
  ...
  try {
    ...
    if (user) {
      ...
      const newUser = new User({
        username:
          req.body.name.split(" ").join("").toLowerCase().slice(0, 4) +
